...
export default function ChatbotClient({
  ...
  const [chatHeaderText] = useState(initialSettings.headerText || defaultHeaderText);
  const [chatThumbnailUrl] = useState(initialSettings.thumbnailUrl || fallbackThumbnail);
  const [chatThumbnailDataUrl] = useState(initialSettings.thumbnailDataUrl || "");
  ...
  const handleSend = async () => {
    ...
    setMessages((prev) => [...prev, { id: crypto.randomUUID(), role: "user", text: trimmed, timestamp: now }]);
    setInput("");
    setLoading(true);
        ...
        {!hasStarted && (
          <div className="flex flex-col items-center gap-6 text-center animate-in fade-in zoom-in duration-700 w-full mb-4">
            ...
            <div className="flex flex-col items-center justify-center gap-4 text-2xl md:text-3xl font-bold tracking-tight text-gray-800 w-full">
              ...
              <span className="text-center whitespace-pre-line leading-tight">{chatHeaderText || defaultHeaderText}</span>
            </div>
          </div>
        )}
          ...
          <div className="w-full flex-1 overflow-y-auto px-2 py-4 space-y-6 scrollbar-hide">
            {messages.map((m) => (
              <div key={m.id} className={`flex w-full ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] md:max-w-[70%] rounded-2xl px-5 py-3.5 text-[15px] leading-relaxed shadow-sm
                     ${m.role === "user" ? "bg-gray-900 text-white rounded-tr-sm" : "bg-gray-100 text-gray-800 rounded-tl-sm border border-gray-200"}`}
                >
          ...
          <div className="relative group">
            ...
            <button
              ...
              className={`absolute right-3 top-1/2 -translate-y-1/2 p-2.5 rounded-full 
                ${loading ? "bg-gray-300" : "bg-black hover:bg-gray-800"} text-white shadow-md transition-all 
                group-focus-within:scale-110 group-hover:scale-105 active:scale-95`}
            >
              <Send size={18} />
            </button>
          </div>
